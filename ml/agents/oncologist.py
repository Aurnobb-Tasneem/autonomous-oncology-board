    ...
    def synthesise(
        ...
        try:
            if self.llm.ping():
                ...
                plan_dict = extract_json_object(text)
                if plan_dict is None:
                    log.warning("OncologistAgent: JSON extraction failed, using rule-based fallback")
        ...
        plan = ManagementPlan(
            ...
            from_fallback=bool(plan_dict.get("from_fallback", False)),
        )
...
Return only the JSON."""
        ...
        try:
            if self.llm.ping():
                ...
                plan_dict = extract_json_object(text)
                if plan_dict is None:
                    log.warning("OncologistAgent: revision JSON extraction failed, using fallback")
...
Be specific, clinically precise, and actionable. 2–4 concerns maximum."""
        ...
        try:
            ...
            if result is None:
                raise ValueError("Could not extract JSON from pathology clarification response")
